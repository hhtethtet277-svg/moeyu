...
# Environment Variables မှ ဖတ်ရန် (မရှိပါက Default Value များဖြင့် အလုပ်လုပ်မည်)
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8971474033:AAHwWl9otpw1rmHhaDTrKQZJkf5FGzxN3iw')
GITHUB_TOKEN = os.environ.get('GITHUB_TOKEN', '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')
REPO_OWNER = os.environ.get('REPO_OWNER', 'hhtethtet277-svg')
REPO_NAME = os.environ.get('REPO_NAME', 'moeyu')
ADMIN_ID = os.environ.get('ADMIN_ID', '7695807003')
...
# CPU Bound ဖြစ်သော OCR ကြောင့် Event Loop မပိတ်စေရန် Semaphore ခံခြင်း
...
async def web_server():
    ...
    await runner.setup()
    # Render သည် 'PORT' variable ကို အလိုအလျောက် သတ်မှတ်ပေးပါသည်
...
async def handle_input(message):
    ...
    if message.chat.id in user_data:
        ...
        if await check_session_url(session_url=url):
            ...
            await bot.reply_to(message, "Session URL အားသိမ်းဆည်းပြီးပါပြီ။ /scan 6, 7, 8, all, ascii-lower စသည်ဖြင့် မိမိအသုံးပြုလိုတာကိုရွေးပြီး စတင်ပါ။")
        else:
...
async def Captcha_Text(image_bytes):
    # OCR Section ကို တစ်ပြိုင်နက်အလုပ်လုပ်မှု အကန့်အသတ်ပေးရန် Semaphore သုံးထားသည်
    async with _ocr_sem:
...
async def verify_captcha(session, session_id, text):
    ...
    async with session.post('https://portal-as.ruijienetworks.com/api/auth/captcha/verify', headers=headers, json=json_data) as req:
        data = await req.json()
        print(f"[verify_captcha] status={req.status} authCode={text} response={data}")
